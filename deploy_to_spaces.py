...
def create_spaces_directory(space_name="medvisor-ai"):
    ...
    if os.path.exists(spaces_dir):
        print(f"WARNING: Directory {spaces_dir} already exists. Removing...")
        shutil.rmtree(spaces_dir)
    
    os.makedirs(spaces_dir)
    print(f"SUCCESS: Created directory: {spaces_dir}")
    ...
    if os.path.exists("requirements_spaces.txt"):
        ...
        print("SUCCESS: Copied: requirements_spaces.txt -> requirements.txt")
...
def setup_git_repository(spaces_dir, space_url):
    ...
    print("SUCCESS: Initialized git repository")
    
    # Add remote
    subprocess.run(["git", "remote", "add", "origin", space_url], check=True)
    print(f"SUCCESS: Added remote: {space_url}")
    
    # Add all files
    subprocess.run(["git", "add", "."], check=True)
    print("SUCCESS: Added all files to git")
    
    # Initial commit
    subprocess.run(["git", "commit", "-m", "Initial MedVisor AI deployment"], check=True)
    print("SUCCESS: Created initial commit")
